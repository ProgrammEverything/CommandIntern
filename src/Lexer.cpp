    ...
    switch (*m_current_char){
        ...
        case '\"': {
            ...
            return flags::Token{
                ...
                str_end - tkn_start,
                flags::Position{m_cchar, m_cline},
        ...
        default:
            if (isdigit(*m_current_char)){ // if current character is digit
                ...
                return flags::Token{std::string_view(start_tkn, size), size,flags::Position{m_cline, m_cchar}, flags::T_Type::EXPRESSION_NUMBER}; // make a string_view with data of integer and type number
                ...
                int size = m_current_char-start_tkn;
                return flags::Token{std::string_view(start_tkn, size), size,flags::Position{m_cline, m_cchar}, flags::T_Type::EXPRESSION_NAME}; // Make a string view with type name and data of the read data
            }
                ...
                throw std::runtime_error("Unkown character " + std::string("'") + std::string(1, *m_current_char) + "'" + " at line " + std::to_string(m_cline) + "At char" + std::to_string(m_cchar));
            }
    }
